...
PHP
		...
		echo "CREATING WooCommerce Address Book PLUGIN DIR AT $WC_PLUGIN_DIR"
		mkdir $WC_PLUGIN_DIR
		echo "COPYING CHECKED OUT BRANCH TO $WC_PLUGIN_DIR"
		cp -R "$TRAVIS_BUILD_DIR" "$WP_CORE_DIR/wp-content/plugins/"
		ls "$WP_CORE_DIR/wp-content/plugins/woo-address-book/"
		...
		echo "ACTIVATING WooCommerce Address Book PLUGIN"
		php wp-cli.phar plugin activate woo-address-book
